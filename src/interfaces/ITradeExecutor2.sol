...
interface ITradeExecutor2 {
    ...
    event TradeInitiated(address indexed user, uint256 indexed nonce, uint256 amount, string intentType);
    event TradeSettled(address indexed user, uint256 indexed nonce);

    function initiateTrade(address user, uint256 amount, string calldata intentType) external returns (uint256);
