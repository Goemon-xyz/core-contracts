...
interface IGoemonCore {
    ...
    event IntentSubmitted(address indexed user, uint256 amount, string intentType);
    event IntentSettled(address indexed user, uint256 intentIndex, int256 pnl);
    event TreasuryUpdated(address newTreasury);

    function permitDeposit(uint160 amount, uint256 deadline, uint48 nonce, bytes calldata signature) external;

    function withdraw(uint256 amount) external;

    function submitIntent(uint256 amount, string calldata intentType) external;

    function settleIntent(address user, uint256 intentIndex, int256 pnl) external;
